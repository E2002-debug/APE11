...
if command -v node &> /dev/null; then
    ...
    echo "[ERROR] Node.js no encontrado. Instalalo con:"
...
echo "---------------------------------------"
echo "[PKG] Paso 1: Compilando Frontend React..."
echo "---------------------------------------"
cd frontend
npm install
npm run build
cd ..
echo ""
echo "[OK] Frontend compilado  src/main/resources/static/"
echo ""

# 2. Compilar Backend Spring Boot
echo "---------------------------------------"
echo "[PKG] Paso 2: Compilando Backend Spring Boot..."
echo "---------------------------------------"
...
if [ -f "./mvnw" ]; then
    ...
    echo "[ERROR] Maven no encontrado. Instalalo con:"
...
if [ -f "$JAR_FILE" ]; then
    ...
    echo "[ERROR] Error: No se generó el archivo JAR"
    exit 1
fi
