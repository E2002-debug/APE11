...
echo "[RED] Tu IP detectada: $LOCAL_IP"
echo ""

# Preguntar el ID del nodo
read -p "[ID] Ingresa el ID de este nodo (1-4): " NODE_ID

if [[ ! "$NODE_ID" =~ ^[1-4]$ ]]; then
    echo "[ERROR] Error: El ID debe ser un número entre 1 y 4"
    exit 1
fi

echo ""
echo "Ingresa las IPs de las 4 computadoras."
...
for i in 1 2 3 4; do
    if [ "$i" -eq "$NODE_ID" ]; then
        ...
        read -p "  PC $i: " IP
        if [ -z "$IP" ]; then
            echo "[ERROR] Error: Debes ingresar la IP de la PC $i"
            exit 1
        fi
    fi
    IPS[$i]=$IP
...
if [ ! -f "$PROPERTIES_FILE" ]; then
    echo "[ERROR] No se encontró application.properties en: $PROPERTIES_FILE"
    exit 1
fi
...
echo ""
echo "[OK] Archivo application.properties actualizado correctamente"
echo ""
...
echo "[DOC] Script de inicio creado: start-node${NODE_ID}.sh"
echo ""
echo "==========================================="
